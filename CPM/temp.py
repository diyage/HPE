...
class LSPDataset(Dataset):
    def __init__(self, root_dir, transform=None, phase_train=True,
                 weighted_loss=False, bandwidth=0.068):
        self.scaled_h = 368
        self.scaled_w = 368
        self.map_h = 45
        self.map_w = 45
        ...
        if lms is None or imagefiles is None or weight is None:
            mat_lsp = scipy.io.loadmat(os.path.join(root_dir, 'joints.mat'),
                                       squeeze_me=True, struct_as_record=False)['joints']
            mat_lspet = scipy.io.loadmat(os.path.join(root_dir, 'joints.mat'),
                                         squeeze_me=True, struct_as_record=False)['joints']
            image_lsp = np.array(glob.glob(root_dir + '/images/*.jpg', recursive=True))

            image_lspet = np.array(glob.glob(root_dir + '/images/*.jpg', recursive=True))

            image_nums_lsp = np.array([float(s[-8:-4]) for s in image_lsp])

            image_nums_lspet = np.array([float(s[-8:-4]) for s in image_lspet])

            sorted_image_lsp = image_lsp[np.argsort(image_nums_lsp)]
            sorted_image_lspet = image_lspet[np.argsort(image_nums_lspet)]

            a = np.transpose(mat_lspet, axes=(2, 1, 0))[:, :2, :]
            b = np.transpose(mat_lsp, axes=(2, 0, 1))[:, :2, :]
            print(1)
            self.lms = np.append(a,
        ...
        if self.transform is None:
            self.transform = transforms.Compose([
                Scale(self.scaled_h, self.scaled_w),
                RandomHSV((0.8, 1.2),
                          (0.8, 1.2),
                          (25, 25)),
                ToTensor()
